...
export const getKPIs = () =>
  client.get<KPI[]>(`/admin/analytics/kpis`);
...
export interface TrainingPathApproval {
  ...
  duration?: string;
  modules?: Array<{ id?: string; title?: string; trainingUnits?: Array<{id?: string; title?: string; type?: string; duration?: string; content?: string}> }>;
  students?: number;
  rating: number;
 ...
 */
export const updateProxmoxServer = (serverId: string, serverData: Partial<ProxmoxServer>) =>
  client.patch<ProxmoxServer>(`/admin/proxmox-servers/${serverId}`, serverData);
