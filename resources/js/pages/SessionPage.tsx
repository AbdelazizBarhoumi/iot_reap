...
> = {
    ...
    provisioning: {
        bg: 'bg-info/10',
        text: 'text-info border-info/30',
        label: 'Provisioning',
    },
    active: {
        bg: 'bg-success/10',
        text: 'text-success border-success/30',
        label: 'Active',
    },
    expiring: {
        bg: 'bg-warning/10',
    ...
    failed: {
        ...
        text: 'text-destructive border-destructive/30',
        label: 'Failed',
    },
    terminated: {
        bg: 'bg-muted',
...
export default function SessionPage({
    ...
    const { session, loading, error, refetch } = useSessionStatus(sessionId, {
        onReady: () => {},
        onExpiring: () => {
    ...
    const effectiveExpiresAt =
        overrideExpiresAt ?? displaySession?.expires_at ?? '';
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: displaySession?.id
                ? `Session ${displaySession.id.substring(0, 8)}…`
                : 'Loading…',
            href: displaySession?.id
                ? `/sessions/${displaySession.id}`
                : '/sessions',
        },
    ];
    // ---------- Loading ----------
            ...
            <div className="min-h-screen bg-background">
                <div className="container py-8">
                    ...
                    <motion.div
                        ...
                        className="mb-6 flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between"
                    >
                        <div className="flex items-center gap-4">
                            <div
                                className={`flex h-12 w-12 items-center justify-center rounded-lg ${statusConfig.bg} ${statusConfig.text.split(' ')[0]}`}
                            >
                                <OSIcon className="h-6 w-6" />
                            </div>
                            <div>
                                <h1 className="font-heading text-3xl font-bold text-foreground">
                                    Session {ds.id.substring(0, 8)}
                                </h1>
                                <div className="mt-2 flex items-center gap-2">
                                    ...
                                    {copied && (
                                        <span className="sr-only text-xs text-success">
                                            Copied!
                                        </span>
                                    )}
                                </div>
                        ...
                        <div className="flex items-center gap-3">
                            <Badge
                                variant="outline"
                                className={`${statusConfig.text} capitalize`}
                            >
                                {statusConfig.label}
                            </Badge>
                    ...
                    {isPending && (
                        <Alert>
                            ...
                            <AlertDescription>
                                Your VM is being provisioned. The remote desktop
                                will appear automatically once the VM is ready…
                            </AlertDescription>
                        </Alert>
                    )}
                    {/* ---- Main grid ---- */}
                                ...
                                <Card className="shadow-card">
                                    ...
                                    <CardContent className="space-y-3">
                                        ...
                                        <InfoRow label="Protocol">
                                            <Badge
                                                variant="outline"
                                                className="border-info/30 bg-info/10 text-info uppercase"
                                            >
                                                {ds.protocol}
                                            </Badge>
                                        </InfoRow>
