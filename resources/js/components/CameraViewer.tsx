...
import { AlertCircle, Maximize2, Minimize2, Settings, Video, WifiOff } from 'lucide-react';
...
export function CameraViewer({
  ...
  const startWebRTC = useCallback(async (): Promise<(() => void) | undefined> => {
    ...
    try {
      ...
      pc.oniceconnectionstatechange = () => {
        if (pc.iceConnectionState === 'failed' || pc.iceConnectionState === 'disconnected') {
          ...
          console.warn('[CameraViewer] WebRTC connection failed, falling back to HLS');
          cleanupWebRTC();
          setMode('hls');
        }
      };
      ...
      const localDescription = await new Promise<RTCSessionDescription>((resolve, reject) => {
      ...
      if (!response.ok) {
        ...
        throw new Error(`WHEP proxy failed: ${response.status} — ${errorText}`);
      }
      ...
      const timeout = setTimeout(() => {
        if (!connected) {
          console.warn('[CameraViewer] WebRTC timeout, falling back to HLS');
          cleanupWebRTC();
          setMode('hls');
        }
      }, 8000);
  ...
  const startHLS = useCallback((): (() => void) | undefined => {
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, () => {
        console.log('[CameraViewer] HLS manifest parsed, starting playback');
        video.play().catch((e) => {
      ...
      hls.on(Hls.Events.ERROR, (_event, data) => {
        console.error('[CameraViewer] HLS error:', data.type, data.details, data.fatal ? '(fatal)' : '');
        if (data.fatal) {
          switch (data.type) {
            case Hls.ErrorTypes.NETWORK_ERROR:
              console.log('[CameraViewer] Attempting to recover from network error');
              hls.startLoad();
              break;
            case Hls.ErrorTypes.MEDIA_ERROR:
              console.log('[CameraViewer] Attempting to recover from media error');
  ...
  }, [mode, camera.status, camera.stream_key]);
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-muted-foreground/30 bg-muted/50 p-8">
        ...
        <p className="text-sm font-medium text-muted-foreground">
          {camera.status === 'active' ? 'Stream connecting...' : 'Camera is inactive'}
        </p>
  ...
  return (
    <div ref={containerRef} className="relative overflow-hidden rounded-lg bg-black">
        ...
        <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/80">
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-xs text-muted-foreground">
              {mode === 'webrtc' ? 'Connecting (WebRTC)...' : 'Buffering (HLS)...'}
            </p>
          </div>
        </div>
      )}
      ...
      {changingResolution && (
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/80">
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-xs text-blue-400">Changing resolution...</p>
          </div>
        </div>
      )}
      ...
      <div className="absolute right-2 top-2 flex items-center gap-1">
        {/* Stream mode toggle */}
        <button
          onClick={() => setMode(mode === 'webrtc' ? 'hls' : 'webrtc')}
          title={mode === 'webrtc' ? 'Switch to HLS (higher latency)' : 'Switch to WebRTC (lower latency)'}
          className="rounded bg-black/60 p-1.5 text-white/80 hover:bg-black/80 hover:text-white"
        >
          <span className="text-[10px] font-bold">{mode === 'webrtc' ? 'HLS' : 'RTC'}</span>
        </button>

        {/* Resolution settings */}
        {resolutions && resolutions.length > 0 && onResolutionChange && (
          <div className="relative">
            ...
            {showSettings && (
              <div className="absolute right-0 top-full z-30 mt-1 w-48 rounded-md bg-gray-900/95 p-1 shadow-xl">
                ...
                >
                  <span>Auto</span>
                  <span className="text-[10px] text-gray-400">Best for your connection</span>
                </button>
                {/* Manual presets */}
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[10px] text-gray-400">
                        {preset.recommended_framerate}fps
                      </span>
                    </button>
                  );
                })}
