...
/**
 * TrainingPathCard only displays a subset of trainingPath properties.
 * This interface allows both full TrainingPath objects and mock data to be used.
 */
interface TrainingPathCardData {
    id: string | number;
    title: string;
    description: string;
    instructor: string;
...
const TrainingPathCard = ({
    ...
    trainingPath: TrainingPathCardData;
    index?: number;
}) => {
    ...
    const IconComponent = categoryIcons[trainingPath.category] || BookOpen;
    return (
        <motion.div
                                ...
                                <div className="flex items-center gap-1">
                                    <div className="flex">
                                        {[...Array(5)].map((_, i) => (
                                            <Star
                                                key={i}
                                                className={`h-3.5 w-3.5 ${i < Math.floor(trainingPath.rating) ? 'fill-amber-400 text-amber-400' : 'fill-muted text-muted'}`}
                                            />
                                        ))}
                                    </div>
