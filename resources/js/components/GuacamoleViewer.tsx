...
import { AlertCircle, Expand, Loader2, Minimize, RefreshCw } from 'lucide-react';
...
function WaitingForSession({ vmIpAddress }: { vmIpAddress?: string | null }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-lg border border-dashed p-12 text-center">
      ...
      {vmIpAddress && (
        <p className="mt-1 text-xs text-muted-foreground">VM IP: {vmIpAddress}</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center gap-1">
        <Button variant="ghost" size="icon" onClick={onReconnect} title="Reconnect">
...
export function GuacamoleViewer({
  ...
  const { tokenData, loading, error, refresh } = useGuacamoleToken(sessionId, isActive);
  ...
  const [connectionState, setConnectionState] = useState<GuacStateValue>(GuacState.IDLE);
  ...
  useEffect(() => {
    ...
    client.onstatechange = (state: number) => {
      ...
      console.debug('[GuacamoleViewer] state →', STATE_LABELS[s] ?? state);
      setConnectionState(s);

      if (s === GuacState.CONNECTING) setConnectionError(null);
      if (s === GuacState.DISCONNECTED) setConnectionError('Remote desktop connection closed.');
    };

    client.onerror = (status: Guacamole.Status) => {
      console.error('[GuacamoleViewer] error', status);
      setConnectionError(`Remote desktop error: ${status?.message ?? 'Unknown error'}`);
    };
    ...
    guacDisplay.onresize = () => fitDisplayToContainer(guacDisplay, containerEl);
  ...
  if ((error || connectionError) && !tokenData) {
    return (
      <ErrorState
        message={error ?? connectionError ?? 'Could not load the remote desktop viewer.'}
        onRetry={refresh}
      />
    );
  }
  ...
  return (
    <div ref={containerRef} className="flex flex-col gap-2 rounded-lg bg-background">
      {/* Toolbar */}
      <Toolbar
        protocol={protocol}
