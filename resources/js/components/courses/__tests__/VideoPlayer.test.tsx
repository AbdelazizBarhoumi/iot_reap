...
describe('VideoPlayer Component', () => {
    ...
    it('renders video with poster image when provided', () => {
        render(<VideoPlayer {...mockProps} />);
        // The poster should be set as an attribute on the video element
        const videoContainer = screen.getByTestId('video-container') || 
                               screen.getByRole('region', { name: /video player/i });
    ...
    it('shows loading state initially', () => {
        render(<VideoPlayer {...mockProps} />);
        // Should show loading indicator before video metadata is loaded
        expect(screen.getByTestId('loading-indicator') || 
               screen.getByLabelText(/loading/i)).toBeInTheDocument();
    });
    it('handles HLS source correctly', () => {
        render(<VideoPlayer {...mockProps} />);
        // HLS should be initialized for .m3u8 sources
