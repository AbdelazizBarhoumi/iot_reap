...
\subsubsubsection{Specification of the Use Case Edit / Delete Proxmox Server}
\begin{table}[H]
    \centering
    \caption{Use Case Specification: Edit / Delete Proxmox Server}
    \begin{tabular}{|p{2.7cm}|p{12.5cm}|}
        \hline
        \textbf{Title}          & Edit / Delete Proxmox Server \\ \hline
        \textbf{Summary}        & Administrator updates the configuration of an existing Proxmox server or removes it from the inventory. Editing updates details like credentials and resource limits; deletion removes the server and its local references from the database. \\ \hline
        \textbf{Actor}          & Administrator \\ \hline
        ...
        \textbf{Post-condition} & If edited, the updated configuration is persisted and audited. If deleted, the server record and related local references are removed and audited. \\ \hline
        \textbf{Basic Scenario} & \begin{minipage}[t]{\linewidth}
                                      \begin{enumerate}[topsep=0pt,itemsep=1pt,parsep=0pt,partopsep=0pt]
                                          \item Administrator selects an existing Proxmox server from the inventory.
                                          \item System validates administrator access, loads the server record, and displays management actions.
                                          \item Administrator chooses either \textit{Edit} or \textit{Delete}.
                                          \item \textbf{If Edit:} Administrator submits updated details, which the system validates.
                                          \item System tests the connection if host, port, or credentials changed, then persists the new configuration and logs the update.
                                          \item \textbf{If Delete:} Administrator confirms deletion. System removes associated local node and session references.
                                          \item System deletes the Proxmox server record and logs the deletion.
                                          \item System returns a success message and refreshes the inventory view.
                                      \end{enumerate}
                                  \end{minipage} \\ \hline
        \textbf{Error Scenario} & \begin{minipage}[t]{\linewidth}
                                      \begin{enumerate}[topsep=0pt,itemsep=1pt,parsep=0pt,partopsep=0pt]
                                          \item If submitted edit data is invalid, the system returns a validation error without persisting changes.
                                          \item If the connection test fails during an edit, the system aborts the update and retains the previous configuration.
                                          \item If the target server does not exist, the system returns a not-found error.
                                          \item If deletion fails (e.g., when removing related local records), the system aborts the operation and preserves the existing data.
                                      \end{enumerate}
                                  \end{minipage} \\ \hline
    \end{tabular}
    \label{tab:admin-edit-delete-proxmox-server}
\end{table}
