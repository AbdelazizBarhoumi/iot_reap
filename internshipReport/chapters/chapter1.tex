...
\textbf{Vision:} The cluster envisions becoming a leading competitiveness hub for Industry 4.0 adoption and digital transformation in North Africa and the broader MENA region, focusing on innovation infrastructure, technical talent development, and sustainable academia-industry partnerships.
...
To address these challenges, Novation City extends its capabilities with secure remote access platforms enabling engineers to interact with industrial systems remotely through the IoT-REAP initiative, combining Infrastructure-as-a-Service provisioning with zero-trust security principles.

\subsection{Academic Framework}

The project fits within the software engineering and Industry 4.0 curriculum, applying web engineering, software architecture, database modeling, remote infrastructure integration, and secure system design to an industrial problem posed by Novation City. The academic evaluation framework requires formal documentation through UML modeling, requirements analysis, implementation reporting, and verification of the delivered platform.
...
\begin{itemize}
    ...
    \item \textbf{Audit Trail:} Insufficient logging for compliance with industrial security standards (IEC 62443) {\footnotesize\cite{iec62443}}
...
\textbf{No OT/IT Network Isolation:} Traditional remote access tools do not enforce Purdue model network segmentation. Engineers accessing OT systems from IT networks create security vulnerabilities that violate IEC 62443 standards {\footnotesize\cite{iec62443}}.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{2.8cm}|p{3.5cm}|p{3.5cm}|p{3.5cm}|}
        ...
        \hline
        Browser-based Access & No, client required     & Client required (limited web exceptions)         & Yes, web dashboards             \\
        \hline
...
These deficiencies demonstrate that current solutions are designed for traditional IT environments and do not satisfy Industry 4.0 requirements for zero-trust security, OT compliance, resource-efficient scheduling, unified management, and predictive operations. Consequently, a purpose-built industrial remote access platform is required.
...
addresses the critical limitations identified in current industrial remote access
solutions, creating a unified platform for Industry 4.0 operations.
...
\textbf{API-First Integration:} External services (Proxmox VE, Apache Guacamole, MQTT broker, AI microservice) integrate through well-defined interfaces, ensuring loose coupling and flexibility to adapt to evolving infrastructure {\footnotesize\cite{proxmox-ve-api} \cite{guacamole-manual} \cite{mqtt-oasis}}.

\textbf{Event-Driven Architecture:} System components communicate via Laravel Events and WebSocket broadcasts, enabling real-time updates for session status, robot telemetry, and security alerts {\footnotesize\cite{laravel-docs}}.

\textbf{Microservices for AI/ML:} Machine learning components (demand forecasting, anomaly detection) are deployed as separate Python FastAPI services, allowing independent scaling and technology-appropriate implementations {\footnotesize\cite{fastapi-docs}}.

\textbf{Zero-Trust Security Model:} Every access request is authenticated and authorized regardless of network location. Sessions are continuously monitored with real-time risk scoring, and high-risk activities trigger automatic termination in accordance with least-privilege principles.
...
\begin{itemize}[label=-]
    \item The platform must provide responsive navigation across training,
          administration, and infrastructure pages with page load times compatible with standard web application thresholds
    \item Video streaming and progress tracking must occur with latency compatible with interactive remote desktop and streaming workloads
    \item VM provisioning and session retrieval must remain usable under concurrent
          demand up to the tested load targets
    \item Search, analytics summaries, and notification retrieval must execute
          within bounded query response times
\end{itemize}

\textbf{Security:}
\begin{itemize}[label=-]
    \item Authentication must rely on secure session-based identity management and verified email
          addresses
    ...
    \item Access to VM sessions, cameras, and USB devices must be restricted to
          authorized users through role-based policies
    \item Sensitive administrative actions must be auditable with immutable log entries
\end{itemize}
...
\textbf{Product Backlog Management:} Features and requirements are maintained in a prioritized Product Backlog of user stories. The Product Owner role prioritizes items based on business value, risk, and dependencies. This allows adaptation: new security requirements or infrastructure changes are incorporated into subsequent sprints.
...
The IoT-REAP platform development followed Agile Scrum principles with a prioritized product backlog organized by sprint. The complete backlog comprises user stories. Table \ref{tab:product-backlog} presents key user stories with priorities and sprint assignments. Sprints 2.5 and 4.5 represent mid-cycle scope extensions identified during backlog refinement, executed without disrupting the main two-week cadence.
...
\multicolumn{4}{|c|}{\textbf{Sprint 1: Foundation \& Authentication}} \\
\hline
...
\hline
05 & As a developer, I need React frontend with TypeScript and Zustand state management {\footnotesize\cite{react-docs} \cite{typescript-docs} \cite{zustand-docs}} & High & Sprint 1 \\
\hline
\multicolumn{4}{|c|}{\textbf{Sprint 2: Proxmox Integration}} \\
\hline
...
\hline
11 & As an admin, I want to register a Proxmox server with encrypted credentials & High & Sprint 2 \\
\hline
\pagebreak[4]
\multicolumn{4}{|c|}{\textbf{Sprint 2.5: Multi-Proxmox Server Support}} \\
\hline
11a & As a developer, I need to extend the Proxmox architecture to support multiple independent clusters & High & Sprint 2.5 \\
\hline
11b & As an engineer, I want to select a target Proxmox cluster when provisioning a VM & High & Sprint 2.5 \\
\hline
...
\hline
\multicolumn{4}{|c|}{\textbf{Sprint 4: File Transfer \& Robot Foundation}} \\
\hline
...
\hline
\multicolumn{4}{|c|}{\textbf{Sprint 4.5: Technical Training Platform}} \\
\hline
41 & As an instructor, I want to create structured courses with modules, lessons, and prerequisite tracking & High & Sprint 4.5 \\
\hline
42 & As an instructor, I want to upload videos with HLS streaming and multi-quality playback (360p-1080p) {\footnotesize\cite{hls-rfc8216}} & High & Sprint 4.5 \\
\hline
...
\hline
\multicolumn{4}{|c|}{\textbf{Sprint 5: Camera \& AI Scheduler}} \\
\hline
...
\hline
\multicolumn{4}{|c|}{\textbf{Sprint 6: Zero-Trust Security --- Feature Complete}} \\
\hline
...
\hline
28 & As a developer, I need predictive maintenance AI for robot health scoring & High & Sprint 6 \\
\hline
\pagebreak[4]
\multicolumn{4}{|c|}{\textbf{Sprint 7: OT Isolation, Catalog \& Compliance}} \\
\hline
29 & As an admin, I want to enforce Purdue model network isolation (IT/OT zones) & High & Sprint 7 \\
\hline
30 & As a developer, I need tamper-proof audit log with SHA-256 hash chain & High & Sprint 7 \\
\hline
31 & As an admin, I want PDF compliance reports for IEC 62443 audits & High & Sprint 7 \\
\hline
32 & As an engineer, I want AI-assisted template suggestions based on my description & Medium & Sprint 7 \\
\hline
33 & As an admin, I want complete dashboard with quota management and activity logs & High & Sprint 7 \\
\hline
\multicolumn{4}{|c|}{\textbf{Sprint 8: Testing}} \\
\hline
34 & As a developer, I need 80\% test coverage with mocked external APIs & High & Sprint 8 \\
\hline
35 & As a developer, I need load testing: 50 concurrent logins, 25 VMs, 40 Guacamole sessions & High & Sprint 8 \\
\hline
36 & As a user, I want comprehensive documentation (user manual, API docs) & High & Sprint 8 \\
\hline
37 & As a developer, I need to prepare and deliver 15-minute live demo & High & Sprint 8 \\
\hline
\end{longtable}
...
Unified Modeling Language (UML) is a standardized visual modeling language used in software engineering to specify, visualize, construct, and document the artifacts of software systems {\footnotesize\cite{booch2005}}. Developed by Grady Booch, James Rumbaugh, and Ivar Jacobson in the mid-1990s, UML provides a common vocabulary for object-oriented software design and is the industry standard for software architecture documentation.

For the IoT-REAP platform, UML modeling was applied during the design phase to produce use case diagrams (Section \ref{sec:requirements}), class diagrams, and sequence diagrams illustrating critical workflows such as VM provisioning and robot command dispatch. These models served as architectural contracts between the developer and stakeholders, ensuring shared understanding of system boundaries before implementation.
...
The IoT-REAP platform development is organized as a phased timeline from February to mid-June 2026, following an Agile Scrum sprint-based approach {\footnotesize\cite{agile}}. The project is structured into eight two-week sprints with a small integration and testing buffer, and it reaches two key milestones at Sprint 3 (MVP Demo) and Sprint 6 (Feature Complete).
...
Establishes project foundation with Laravel 12 and React 18 integration via Breeze. Implements session-based authentication, role-based access control (engineer, admin, security\_officer), and database schema with ULID primary keys. Sets up MySQL, Redis, and Docker development environment {\footnotesize\cite{mysql-docs} \cite{redis-docs} \cite{docker-docs}}.
...
Deploys Python FastAPI microservice for ML-based demand forecasting using RandomForestRegressor. Integrates predictions with Laravel scheduler for VM pre-warming. Adds live ESP32-CAM video feed and idle session hibernation {\footnotesize\cite{fastapi-docs} \cite{laravel-docs}}.
...
Enforces Purdue model network isolation with IT/OT zone separation. Builds tamper-proof audit log with SHA-256 hash chain for IEC 62443 compliance. Implements PDF compliance report generator and AI-assisted template catalog {\footnotesize\cite{iec62443}}.
...
\caption{IoT-REAP Platform Development Timeline and Milestones}
...
This chapter has presented the project context, analyzed current remote access limitations, introduced the IoT-REAP solution, specified the functional and non-functional requirements, and described the Agile Scrum methodology and sprint planning adopted to structure the implementation.
